...
const SingleProduct = () => {
  ...
  return (
    <section>
      <div className="textarea-md breadcrumbs">
        <ul>
          <li>
            <Link to='/'>Home</Link>
          </li>
        ...
        <div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-2">
              {colors.map((color) => {
                return (
                  <button key={color} type="button" className={`badge w-6 h-6 mr-2 ${color === productColor && 'border-2 border-primary'}`} style={{ backgroundColor: color }} onClick={() => setProductColor(color)}></button>
                )
              })}
            </div>
          </div>
